...
class TestBlockUnmappableAggs:
    """Teil 12.3: the fail-closed gate on unmappable aggregations must default
    to the strictest behaviour (block), and permissive modes are explicit
    opt-in only — a typo refuses to start rather than silently serving raw."""

    def test_defaults_to_block(self) -> None:
        assert AnonymizationConfig.from_env().block_unmappable_aggs == "block"
    ...
    def test_env_parsing(
        ...
        monkeypatch.setenv("KLAXON_ANONYMIZATION_BLOCK_UNMAPPABLE_AGGS", raw)
        assert AnonymizationConfig.from_env().block_unmappable_aggs == expected

    @pytest.mark.parametrize("bad", ["blok", "maybe", "truefalse", "block-drop"])
    def test_invalid_value_raises(
        self, monkeypatch: pytest.MonkeyPatch, bad: str
    ) -> None:
        monkeypatch.setenv("KLAXON_ANONYMIZATION_BLOCK_UNMAPPABLE_AGGS", bad)
    ...
    def test_yaml_block(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
        ...
        path.write_text(
            "anonymization:\n  block_unmappable_aggs: drop\n", encoding="utf-8"
        )
        monkeypatch.setenv("KLAXON_CONFIG", str(path))
        assert AnonymizationConfig.from_env().block_unmappable_aggs == "drop"
