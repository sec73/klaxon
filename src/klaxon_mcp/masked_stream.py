...
def _painless_script(cfg: TenantConfig) -> str:
    ...
    patterns = "\n".join(
        f'    Pattern {name} = Pattern.compile({json.dumps(_PATTERNS[name])});'
        for name in _FREETEXT_PATTERN_ORDER
    )
    pattern_uses = "\n".join(
        f'        out = maskPattern({name}, out, "{_MASK_FAMILY[name]}");'
